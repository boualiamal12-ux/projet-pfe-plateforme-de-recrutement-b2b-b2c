...
const styles = `
  ...
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── Sort dropdown headers ── */
  .go-sort-th { user-select: none; white-space: nowrap; }
  .go-sort-th-inner { display: inline-flex; align-items: center; gap: 6px; cursor: pointer; padding: 4px 8px; border-radius: 8px; transition: background 0.15s; }
  .go-sort-th-inner:hover { background: rgba(20,184,166,0.08); color: var(--primary-cyan); }
  .go-sort-th-inner.active { color: var(--primary-cyan); }
  .go-sort-chevron { display: flex; align-items: center; transition: transform 0.2s; }
  .go-sort-chevron.open { transform: rotate(180deg); }
  .go-sort-dropdown { position: absolute; top: calc(100% + 6px); left: 0; background: #fff; border: 1.5px solid #e2e8f0; border-radius: 14px; box-shadow: 0 10px 32px rgba(0,0,0,0.12); z-index: 300; overflow: hidden; min-width: 200px; }
  .go-sort-option { display: flex; align-items: center; gap: 10px; padding: 11px 16px; font-size: 13px; font-weight: 600; color: #334155; cursor: pointer; transition: background 0.15s; }
  .go-sort-option:hover { background: #f8fafc; }
  .go-sort-option.selected { background: rgba(20,184,166,0.08); color: var(--primary-cyan); font-weight: 800; }
  .go-sort-option-icon { font-size: 15px; }
  .go-sort-wrap { position: relative; display: inline-block; }

  /* ── Candidate hover tooltip ── */
  .go-cand-tooltip-wrap { position: relative; display: inline-block; }
  .go-cand-tooltip { position: absolute; top: 50%; left: calc(100% + 12px); transform: translateY(-50%); background: #fff; border: 1.5px solid #e2e8f0; border-radius: 16px; box-shadow: 0 12px 40px rgba(0,0,0,0.14); padding: 16px 18px; min-width: 240px; z-index: 200; pointer-events: none; }
  .go-cand-tooltip::before { content: ''; position: absolute; left: -8px; top: 50%; transform: translateY(-50%); border: 6px solid transparent; border-right-color: #e2e8f0; }
  .go-cand-tooltip::after { content: ''; position: absolute; left: -6px; top: 50%; transform: translateY(-50%); border: 5px solid transparent; border-right-color: #fff; }
  .go-cand-tooltip-name { font-size: 14px; font-weight: 800; color: #0f172a; margin-bottom: 4px; }
  .go-cand-tooltip-row { display: flex; align-items: center; gap: 7px; font-size: 12px; color: #64748b; margin-top: 5px; }
  .go-cand-tooltip-avatar { width: 42px; height: 42px; border-radius: 12px; background: linear-gradient(135deg, #14b8a6, #0891b2); display: flex; align-items: center; justify-content: center; color: #fff; font-size: 17px; font-weight: 900; flex-shrink: 0; margin-bottom: 10px; }
`;
...
const GestionOffres = ({ onLogout }) => {
  ...
  const [openSortField, setOpenSortField] = useState(null);
  const sortDropdownRef = useRef(null);

  // ── Candidate real names cache ──
  const [candNames, setCandNames] = useState({});

  // ── Tooltip hover ──
  const [hoveredCandId, setHoveredCandId] = useState(null);
  ...
  useEffect(() => {
    const handler = e => { if (sortDropdownRef.current && !sortDropdownRef.current.contains(e.target)) setOpenSortField(null); };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  }, []);
  ...
  // ── Fetch real candidate names when allCandidatures changes ──
  useEffect(() => {
    const fetchNames = async () => {
      const uniqueIds = [...new Set(allCandidatures.map(c => c.candidatId))];
      const toFetch = uniqueIds.filter(id => !candNames[id]);
      if (toFetch.length === 0) return;
      const results = {};
      await Promise.all(toFetch.map(async id => {
        try {
          const res = await api.get(`/candidats/${id}`);
          const d = res.data;
          results[id] = { prenom: d.prenom || "", nom: d.nom || "", email: d.email || "", telephone: d.telephone || "", ville: d.ville || "" };
        } catch { results[id] = {}; }
      }));
      setCandNames(prev => ({ ...prev, ...results }));
    };
    if (allCandidatures.length > 0) fetchNames();
  }, [allCandidatures]);
  ...
  const filteredCandBase = allCandidatures.filter(c => {
  ...
  const statutOrderAsc  = { ACCEPTEE: 0, EN_ATTENTE: 1, REFUSEE: 2 };
  const statutOrderRefus = { REFUSEE: 0, EN_ATTENTE: 1, ACCEPTEE: 2 };
  const filteredCand = [...filteredCandBase].sort((a, b) => {
    if (!sortField) return 0;
    let va, vb;
    if (sortField === "date") { va = new Date(a.dateEnvoi).getTime(); vb = new Date(b.dateEnvoi).getTime(); }
    else if (sortField === "score") { va = a.score ?? -1; vb = b.score ?? -1; }
    else if (sortField === "statut") { va = statutOrderAsc[a.statut] ?? 99; vb = statutOrderAsc[b.statut] ?? 99; }
    else if (sortField === "statut_refusee") { va = statutOrderRefus[a.statut] ?? 99; vb = statutOrderRefus[b.statut] ?? 99; }
    if (va < vb) return sortDir === "asc" ? -1 : 1;
    if (va > vb) return sortDir === "asc" ? 1 : -1;
    return 0;
  });

  const applySortOption = (field, dir) => {
    setSortField(field);
    setSortDir(dir);
  ...
  const sortOptions = {
    date: [
      { dir: "desc", label: "Plus récentes d'abord", icon: "🆕" },
      { dir: "asc",  label: "Plus anciennes d'abord", icon: "📅" },
    ],
    score: [
      { dir: "desc", label: "Score le plus élevé", icon: "⬆️" },
      { dir: "asc",  label: "Score le plus bas", icon: "⬇️" },
    ],
    statut: [
      { dir: "asc",  label: "Acceptées en premier", icon: "✅" },
      { dir: "desc", label: "En attente en premier", icon: "⏳" },
      { dir: "none", label: "Refusées en premier",  icon: "❌" },
    ],
  };

  const SortDropdownTh = ({ field, label }) => {
    const isOpen = openSortField === field;
    const isActive = sortField === field;
    const opts = sortOptions[field] || [];
    return (
      <th className="go-sort-th">
        <div className="go-sort-wrap" ref={isOpen ? sortDropdownRef : null}>
          <div
            className={`go-sort-th-inner${isActive ? " active" : ""}`}
            onClick={e => { e.stopPropagation(); setOpenSortField(isOpen ? null : field); }}
          >
            {label}
            <span className={`go-sort-chevron${isOpen ? " open" : ""}`}>
              <svg width="10" height="6" viewBox="0 0 10 6"><path d="M1 1L5 5L9 1" stroke={isActive ? "#14b8a6" : "#94a3b8"} strokeWidth="1.8" fill="none" strokeLinecap="round"/></svg>
            </span>
          </div>
          {isOpen && (
            <div className="go-sort-dropdown" onClick={e => e.stopPropagation()}>
              {opts.map(opt => {
                const selected = isActive && (opt.dir === "none" ? sortDir === "none" : sortDir === opt.dir);
                return (
                  <div
                    key={opt.dir}
                    className={`go-sort-option${selected ? " selected" : ""}`}
                    onClick={() => {
                      if (opt.dir === "none") { setSortField("statut_refusee"); setSortDir("asc"); setOpenSortField(null); }
                      else applySortOption(field, opt.dir);
                    }}
                  >
                    <span className="go-sort-option-icon">{opt.icon}</span>
                    {opt.label}
                    {selected && <span style={{ marginLeft: "auto", color: "#14b8a6" }}>✓</span>}
                  </div>
                );
              })}
              {isActive && (
                <div
                  className="go-sort-option"
                  style={{ borderTop: "1px solid #f1f5f9", color: "#94a3b8", fontWeight: 500 }}
                  onClick={() => { setSortField(null); setSortDir("asc"); setOpenSortField(null); }}
                >
                  <span className="go-sort-option-icon">↩️</span> Annuler le tri
                </div>
              )}
            </div>
          )}
  ...
  const getCandName = (c) => {
    // Priorité 1 : champs enrichis dans la candidature (nouveau backend)
    const directFull = [c.candidatPrenom, c.candidatNom].filter(Boolean).join(" ");
    if (directFull) return directFull;
    if (c.candidatEmail) return c.candidatEmail;
    // Priorité 2 : cache candNames
    const d = candNames[c.candidatId];
    if (!d) return `Candidat #${c.candidatId}`;
    const full = [d.prenom, d.nom].filter(Boolean).join(" ");
    if (full) return full;
    if (d.email) return d.email;
    return `Candidat #${c.candidatId}`;
  };

  // Helpers vue détail
  const getInitials = data => {
    if (data?.prenom) return data.prenom[0].toUpperCase();
    if (data?.email) return data.email[0].toUpperCase();
  ...
  const getFullName = data => {
    ...
    return `Candidat #${data?.candidature?.candidatId}`;
  };
              ...
              {loading ? <div style={{ padding: 40, textAlign: "center", color: "#64748b" }}>Chargement...</div> : (
                <table className="go-table">
                  <thead><tr>
                    <th>Candidat</th>
                    <th>Offre</th>
                    <SortDropdownTh field="date" label="Date" />
                    <SortDropdownTh field="score" label="Score IA" />
                    <SortDropdownTh field="statut" label="Statut" />
                    ...
                    {filteredCand.length === 0
                      ...
                      : filteredCand.map(c => (
                        <tr key={c.id} onClick={() => { setSelectedCand(c); setShowCandModal(true); }}>
                          {/* ✅ Clic nom → page dédiée + tooltip hover */}
                          <td>
                            <div
                              className="go-cand-tooltip-wrap"
                              onMouseEnter={() => setHoveredCandId(c.id)}
                              onMouseLeave={() => setHoveredCandId(null)}
                            >
                              <button
                              ...
                              {hoveredCandId === c.id && candNames[c.candidatId] && (
                                <div className="go-cand-tooltip">
                                  <div className="go-cand-tooltip-avatar">
                                    {(candNames[c.candidatId].prenom || "?")[0].toUpperCase()}
                                  </div>
                                  <div className="go-cand-tooltip-name">{getCandName(c)}</div>
                                  {candNames[c.candidatId].email && <div className="go-cand-tooltip-row"><Mail size={12} color="#14b8a6" />{candNames[c.candidatId].email}</div>}
                                  {candNames[c.candidatId].telephone && <div className="go-cand-tooltip-row"><Phone size={12} color="#14b8a6" />{candNames[c.candidatId].telephone}</div>}
                                  {candNames[c.candidatId].ville && <div className="go-cand-tooltip-row"><MapPin size={12} color="#14b8a6" />{candNames[c.candidatId].ville}</div>}
                                  <div className="go-cand-tooltip-row"><Briefcase size={12} color="#14b8a6" />{c.offreTitre}</div>
                                </div>
                              )}
                            </div>
                          </td>
