...
const styles = `
  ...
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── HOVER TOOLTIP ── */
  .ad-ent-card-wrap { position: relative; }
  .ad-ent-tooltip {
    position: absolute; bottom: calc(100% + 10px); left: 50%; transform: translateX(-50%);
    background: #0f172a; color: #e2e8f0; border-radius: 16px; padding: 18px 20px;
    min-width: 280px; max-width: 320px; z-index: 200;
    box-shadow: 0 16px 48px rgba(0,0,0,0.25);
    pointer-events: none;
    animation: tooltipIn 0.18s ease;
  }
  .ad-ent-tooltip::after {
    content: ''; position: absolute; top: 100%; left: 50%; transform: translateX(-50%);
    border: 7px solid transparent; border-top-color: #0f172a;
  }
  .ad-ent-tooltip-title { font-size: 13px; font-weight: 800; color: var(--primary); margin-bottom: 12px; display: flex; align-items: center; gap: 6px; }
  .ad-ent-tooltip-row { display: flex; justify-content: space-between; align-items: center; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.06); font-size: 12px; }
  .ad-ent-tooltip-row:last-child { border-bottom: none; }
  .ad-ent-tooltip-key { color: #94a3b8; font-weight: 500; }
  .ad-ent-tooltip-val { color: #f1f5f9; font-weight: 700; text-align: right; max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  .ad-ent-tooltip-loading { color: #94a3b8; font-size: 12px; text-align: center; padding: 8px 0; }
...
/* ── Tooltip affiché au survol d'une carte entreprise ── */
function useEntDetail(id, enabled) {
  const [detail, setDetail] = useState(null);
  const [loading, setLoading] = useState(false);
  const cache = useRef({});
  useEffect(() => {
    if (!enabled || !id) return;
    if (cache.current[id]) { setDetail(cache.current[id]); return; }
    setLoading(true);
    const token = localStorage.getItem("token");
    axios.get(`http://localhost:8081/api/admin/all`, { headers: { Authorization: token } })
      .then(res => {
        const ent = res.data.find(e => e.id === id);
        if (ent) { cache.current[id] = ent; setDetail(ent); }
      })
      .catch(() => {})
...
function EntTooltip({ ent }) {
  const rows = [
    { key: "Email",        val: ent.email },
    { key: "Secteur",      val: ent.secteur || "—" },
    { key: "Ville",        val: ent.ville || "—" },
    { key: "Taille",       val: ent.taille || "—" },
    { key: "Téléphone",    val: ent.telephone || "—" },
    { key: "Site web",     val: ent.siteWeb || "—" },
    { key: "Description",  val: ent.description ? ent.description.slice(0, 60) + (ent.description.length > 60 ? "…" : "") : "—" },
    { key: "Offres",       val: ent.nombreOffres ?? 0 },
    { key: "Candidatures", val: ent.nombreCandidatures ?? 0 },
    { key: "Statut",       val: ent.validated ? "✓ Validé" : "⏳ En attente" },
  ];
  return (
    <div className="ad-ent-tooltip">
      <div className="ad-ent-tooltip-title">
        <Building2 size={13}/> {ent.nomEntreprise || ent.email}
      </div>
      {rows.map(r => (
        <div className="ad-ent-tooltip-row" key={r.key}>
          <span className="ad-ent-tooltip-key">{r.key}</span>
          <span className="ad-ent-tooltip-val">{r.val}</span>
        </div>
      ))}
    </div>
  );
            ...
            {filteredEntreprises.length===0
              ...
              : <div className="ad-cards-grid">
                  {filteredEntreprises.map(ent=>(
                    <div className="ad-ent-card-wrap" key={ent.id}
                      onMouseEnter={()=>setHoveredId(ent.id)}
                      onMouseLeave={()=>setHoveredId(null)}>
                      {hoveredId===ent.id && <EntTooltip ent={ent}/>}
                    <div className="ad-ent-card">
